	...
	for(var index = 0; index < propArray.length; index++)
	{
		switch(propArray[index])
		{
			...
			case "textfeedback":
				if (objQuestion.strOurAns && objQuestion.strOurAns != '~~~null~~~')
				{
					//LD-8222 - updated conditions to display the correct/incorrect and feedback-by-choice accordingly 
					if (objQuestion.fbByChoices == 0 && (objQuestion.isCorrect() ? objQuestion.corrFeedback : objQuestion.incorrFeedback))
					...
					else if (objQuestion.fbByChoices == 1 && objQuestion.arChoiceFbs && (objQuestion.arChoices.indexOf(objQuestion.strOurAns) != -1) && (objQuestion.arChoiceFbs[objQuestion.arChoices.indexOf(objQuestion.strOurAns)]) != "_blank")
						tempStr += '<p class="' + this.name + settingName + '"><span class="' + this.name + propArray[index] + settingName + '">' + trivstrFDB + ': ' + (objQuestion.arChoiceFbs[objQuestion.arChoices.indexOf(objQuestion.strOurAns)]) + '</span></p>';
				}
			break;
