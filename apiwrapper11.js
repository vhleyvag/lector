...
// ---------------------------------------------------------------------------------
// These are constants and DO NOT SET THEIR VALUES HERE!!
// They must be set in initConsts(), otherwise they do not get set in SPP publish.
//
var _Debug;       // set this to false to turn debugging off
var _NoError;
var _GeneralError;
var _ServerBusy;
var _InvalidArgumentError;
var _ElementCannotHaveChildren;
var _ElementIsNotAnArray;
var _NotInitialized;
var _NotImplementedError;
var _InvalidSetValue;
var _ElementIsReadOnly;
var _ElementIsWriteOnly;
var _IncorrectDataType;
// ---------------------------------------------------------------------------------
...
function LMSSetValue(name, value)
{
   ...
   else
   {
      ...
      LMSCommitOnDelay(); // call even if undefined is returned, some LMS systems do that
   }

   return;
}

function LMSCommitOnDelay()
{
    if (window.lmsCommitST)
        return;

    window.lmsCommitST = setTimeout(
        function () {
            try {
                window.lmsCommitST = false;
                LMSCommit();
            } catch (e) { }
        }
        , 1000);
...
function LMSCommit()
{
   ...
   else
   {
      ...
      {
         if (window.console)
             console.log('LMSCommit() returned an invalid value, expected "true" or "false" and received this: ' + result);
         result = "true"; // LMSCommit return value is ignored by Captivate, Storyline, etc. and some LMS vendors don't respect the SCORM 1.2 Specification
      }
      else if (result.toString() != "true")
      {
         //var err = ErrorHandler("LMSCommit Error: ");
         ErrorHandler("LMSCommit Error: ");
      }
   }
...
function initConsts()
{
    _Debug = false;                         // set this to false to turn debugging off and get rid of those annoying alert boxes.
